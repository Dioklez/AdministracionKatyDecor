...
import '../../services/factura_emitida_service.dart';
...
class _FacturasScreenState extends State<FacturasScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      final list = await FacturaEmitidaService().getAll();
      if (!mounted) return;
      setState(() {
        _all = list;
        _loading = false;
        if (_selectedId != null &&
            !list.any((f) => f.id == _selectedId)) {
          _selectedId = null;
        }
      });
    } catch (e) {
      if (mounted) {
        setState(() {
          _error = 'No se pudo cargar las facturas.';
          _loading = false;
        });
      }
    }
  ...
  List<FacturaEmitida> get _filtered {
    if (_statusFilter == 'all') return _all;
    return _all.where((f) => f.status == _statusFilter).toList();
  }

  FacturaEmitida? get _selected =>
      _selectedId == null
          ? null
          : _all.where((f) => f.id == _selectedId).firstOrNull;
              ...
              : _error != null
                  ...
                  : _filtered.isEmpty
                      ? EmptyState(
                          ...
                          title: 'Sin facturas',
                          subtitle: 'Crea la primera factura emitida',
                          actionLabel: 'Nueva factura',
                          onAction: _showFacturaDialog,
                        )
                      : ListView.builder(
                          itemCount: _filtered.length,
                          itemBuilder: (ctx, i) => _FacturaTile(
                            factura: _filtered[i],
                            selected: _selectedId == _filtered[i].id,
                            onTap: () =>
                                setState(() => _selectedId = _filtered[i].id),
                            onEdit: () =>
                                _showFacturaDialog(edit: _filtered[i]),
        ...
        child: Row(
          children: [
            ...
            _TabBtn(
              ...
              selected: _statusFilter == 'pagada',
              onTap: () => setState(() => _statusFilter = 'pagada'),
            ),
            const SizedBox(width: 6),
            _TabBtn(
              label: 'Canceladas',
  ...
  Widget _buildRightPanel() {
    ...
    if (f == null) {
      return const EmptyState(
        ...
        subtitle: 'Haz clic en una factura para ver su detalle',
      );
    }
    return SingleChildScrollView(
      padding: const EdgeInsets.all(24),
      child: _buildDetail(f),
    );
  }

  Widget _buildDetail(FacturaEmitida f) {
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      f.folio?.isNotEmpty == true ? f.folio! : 'Sin folio',
                      style: GoogleFonts.inter(
                        fontSize: 18,
                    ...
                    if (f.clientName != null && f.clientName!.isNotEmpty)
                      Text(
                        f.clientName!,
                        style: GoogleFonts.inter(
                            fontSize: 13,
              ...
              _StatusBadge(label: f.statusLabel, color: statusColor),
              const SizedBox(width: 8),
              PopupMenuButton<String>(
                icon: Icon(Icons.more_vert,
                    size: 18, color: AppTheme.colorTextoSecundario),
                onSelected: (v) {
                  if (v == 'edit') _showFacturaDialog(edit: f);
          ...
          Wrap(
            ...
            runSpacing: 12,
            children: [
              if (f.clientRfc != null && f.clientRfc!.isNotEmpty)
                _InfoCell(label: 'RFC', value: f.clientRfc!),
              if (f.date != null)
                _InfoCell(label: 'Fecha', value: _fmtDate(f.date)),
              if (f.projectId != null)
                _InfoCell(label: 'Proyecto', value: f.projectId!),
              _InfoCell(label: 'Subtotal', value: _fmtMoney(f.subtotal)),
              _InfoCell(label: 'IVA', value: _fmtMoney(f.tax)),
              _InfoCell(
                label: 'Total',
                value: _fmtMoney(f.total),
          ...
          if (f.items.isNotEmpty) ...[
            ...
            ...f.items.map((item) => _ItemRow(item: item)),
          ],
        ],
      ),
    );
  ...
  void _showFacturaDialog({FacturaEmitida? edit}) {
    showDialog(
      context: context,
      builder: (ctx) => _FacturaDialog(
        editFactura: edit,
        onSaved: () {
  ...
  Future<void> _confirmDelete(FacturaEmitida f) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(
            '¿Eliminar factura${f.folio != null ? ' ${f.folio}' : ''}?',
            style: GoogleFonts.inter(
    ...
    if (confirmed == true && mounted) {
      try {
        await FacturaEmitidaService().delete(f.id);
        if (mounted) {
          setState(() {
            if (_selectedId == f.id) _selectedId = null;
          });
          _loadData();
        }
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('No se pudo eliminar la factura.')),
          );
        }
      }
    }
...
class _FacturaTile extends StatefulWidget {
  final FacturaEmitida factura;
  final bool selected;
  final VoidCallback onTap;
  ...
  @override
  State<_FacturaTile> createState() => _FacturaTileState();
}

class _FacturaTileState extends State<_FacturaTile> {
  bool _hovered = false;
...
class _ItemRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final price =
        item['price']?.toString() ?? item['unit_price']?.toString() ?? '';
...
class _FacturaDialog extends StatefulWidget {
  final FacturaEmitida? editFactura;
  final VoidCallback onSaved;

  const _FacturaDialog({this.editFactura, required this.onSaved});
...
class _FacturaDialogState extends State<_FacturaDialog> {
  ...
  late TextEditingController _clientRfcCtrl;
  ...
  bool get _isEditing => widget.editFactura != null;

  @override
  void initState() {
    super.initState();
    final f = widget.editFactura;
    _folioCtrl = TextEditingController(text: f?.folio ?? '');
    _clientNameCtrl = TextEditingController(text: f?.clientName ?? '');
    _clientRfcCtrl = TextEditingController(text: f?.clientRfc ?? '');
    _projectIdCtrl = TextEditingController(text: f?.projectId ?? '');
    _subtotalCtrl = TextEditingController(
        text: f != null ? f.subtotal.toStringAsFixed(2) : '');
    _taxCtrl = TextEditingController(
        text: f != null ? f.tax.toStringAsFixed(2) : '');
    _totalCtrl = TextEditingController(
        text: f != null ? f.total.toStringAsFixed(2) : '');
    _notesCtrl = TextEditingController(text: f?.notes ?? '');
    _status = f?.status ?? 'pendiente';
    _date = f?.date != null ? DateTime.tryParse(f!.date!) : null;
  }
  ...
  Future<void> _save() async {
    ...
    final body = <String, dynamic>{
      ...
      'clientRfc': _clientRfcCtrl.text.trim(),
    ...
    try {
      final svc = FacturaEmitidaService();
      if (_isEditing) {
        await svc.update(widget.editFactura!.id, body);
      } else {
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      child: TextFormField(
                        controller: _clientRfcCtrl,
                        decoration:
                            const InputDecoration(labelText: 'RFC (opcional)'),
                      ),
                    ),
                  ],
                ),
      ...
      actions: [
        ...
        ElevatedButton(
          ...
          child: _saving
              ...
              : Text(_isEditing ? 'Guardar cambios' : 'Crear factura'),
        ),
      ],
    );
  }
